...
def dm_fraud_update(conn, date):
    load_dt = datetime.datetime.strptime(date, '%d%m%Y').date()
    with conn:
        cur = conn.cursor()

        # удаляем все записи из БД на дату инкремента, чтобы не было дублей
        cur.execute(f'''
            DELETE FROM rep_fraud
            WHERE date(event_dt) = '{load_dt}';
        ''')
        ...
        cur.execute(f'''
            ...
            WHERE date(dt.transaction_date) = '{load_dt}'
